...
function getAllMediaFromArticle(article) {
    ...
    slideContainers.forEach(container => {
        // Cari semua gambar dan video dalam slide
        const mediaElements = container.querySelectorAll('img[src*="instagram"], video[src*="instagram"]');
    ...
    if (mediaUrls.length === 0) {
        const singleMedia = article.querySelectorAll('._aagv img, ._aagv video, ._ab8w img, ._ab8w video');
    ...
    try {
        mediaUrls.forEach((media, index) => {
            chrome.runtime.sendMessage({
                action: "download",
                url: media.url,
                filename: `instagram-${media.isVideo ? 'video' : 'photo'}-${Date.now()}-${index + 1}${media.isVideo ? '.mp4' : (media.isWebP ? '.webp' : '.jpg')}`,
                isWebP: media.isWebP
            });
        });
    } catch (e) {
